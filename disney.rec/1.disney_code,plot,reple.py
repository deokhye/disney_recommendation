...
final_movie_data_plot = []

#영화 줄거리
for codes in movie_codes:
    response = requests.get('https://movie.naver.com/movie/bi/mi/basic.nhn?code='+str(codes))
    soup = BeautifulSoup(response.text, 'html.parser')

    plot = soup.select(
        '#content > div.article > div.section_group.section_group_frst > div.obj_section > div > div > p')


    for line in plot:
        movies_plot = line.text
        print(movies_plot)
        ...
        final_movie_data_plot.append(movie_data)
#print(final_movie_data_plot)
...
with open('movies_title.csv', 'w', encoding='utf-8', newline='') as writer_csv:
...
#코드, 줄거리
field_name_list =['code', 'plot']
with open('movies_plot.csv', 'w', encoding='utf-8', newline='') as writer_csv:
    writer = csv.DictWriter(writer_csv, fieldnames=field_name_list)  
    # writer.writeheader()
    for i in range(0, len(final_movie_data_plot)):
        writer.writerow(final_movie_data_plot[i])
